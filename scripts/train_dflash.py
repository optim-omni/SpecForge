...
from specforge.phased_optimizer import PhasedBF16Optimizer
...
def parse_args():
    ...
    model_group.add_argument(
        ...
        type=float,
        default=0.5,
        help="Weight for KL divergence loss (draft vs target distribution). 0 disables.",
    )
    model_group.add_argument(
        "--phased-training",
        action="store_true",
        help="Enable 3-phase training: fc-only -> fc+layers(low lr) -> shared lr",
    )
    model_group.add_argument(
        "--layer-lr-ratio",
        type=float,
        default=0.1,
        help="Layer LR = fc_lr * this ratio in phase 2",
    )
...
def main():
    ...
    if args.phased_training:
        fc_lr = args.learning_rate * 10  # fc gets 10x LR (default: 6e-4 * 10 = 6e-3)
        optimizer = PhasedBF16Optimizer(
            draft_model,
            fc_lr=fc_lr,
            layer_lr=args.learning_rate,
    ...
    if resume_state is not None:
        ...
        del resume_state
        print_on_rank0(f"Restored optimizer, lr={optimizer.get_learning_rate():.6f}")
