...
class SGLangDFlashTargetModel(DFlashTargetModel):
    ...
    def _extend(self, reqs):
        ...
        # Extract last-layer hidden states for KL loss
        last_hidden_list = None
        if hasattr(output, "hidden_states") and output.hidden_states is not None:
            last_hidden_list = torch.split(output.hidden_states, input_lens, dim=0)
    ...
    def generate_dflash_data(
        ...
        if last_hidden_list is not None:
            last_hidden = torch.cat([h.unsqueeze(0) for h in last_hidden_list], dim=0)
        ...
        return DFlashTargetOutput(
            ...
            loss_mask=loss_mask,
            logits=last_hidden,  # reuse logits field for last_hidden
        )
